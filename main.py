    ...
    if opcion == '1':
        ...
        for index, fila in df_nuevos.iterrows():
            ...
            for item in herramientas_lista: 
                diccionario_herramientas = {
                    ...
                    "Nombre Herramienta": formatear_herramienta(item[0]),
                    "Entrada/Salida": formatear_movimiento(entrada_salida),
                    "Cantidad": item[1],
                    "Empresa": formatear_empresa(empresa),
                    "Persona (Opcional)": formatear_persona(persona)
                }  
        ...
        if not os.path.exists(ruta_json):
            ...
            for item in datos_json:
                movimiento_limpio = formatear_movimiento(item.get('Movimiento', 'Entrada'))
                cantidad = int(item.get('Cantidad', 0))
                if movimiento_limpio == 'Salida':
                    cantidad = cantidad * -1

                diccionario_herramientas = {
                                "Fecha Entrada/Salida": item.get('Fecha_Ingreso', dt.now().strftime("%d/%m/%Y")),
                                "Hora Entrada/Salida": item.get('Hora_Entrada/Salida', None), 
                                "Nombre Herramienta": formatear_herramienta(item.get('Herramienta', '')),
                                "Entrada/Salida": movimiento_limpio,
                                "Cantidad": cantidad,
                                "Empresa": formatear_empresa(item.get('Empresa', None)),
                                "Persona (Opcional)": formatear_persona(item.get('Responsable', None))
                            }
